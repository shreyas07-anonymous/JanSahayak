...
# ==========================================
    ...
    if not history:
        ...
        location_counts = pd.DataFrame(history)['location'].value_counts().head(10)
        fig = px.bar(
        ...
        fig.update_layout(showlegend=False, xaxis_title="Number of Complaints", yaxis_title="Location")
        st.plotly_chart(fig, use_container_width=True)
        
        # Risk Analysis
        st.markdown("### ⚠️ Risk Score Distribution")
        risk_scores = [h['risk_data']['risk_index'] for h in history]
        fig = px.histogram(
            x=risk_scores,
            nbins=20,
        ...
        display_df = pd.DataFrame({
            "ID": [h['complaint_id'] for h in history],
            "Location": [h['location'] for h in history],
            "Issue": [h['issue_type'] for h in history],
            "Risk": [h['risk_data']['risk_index'] for h in history],
            "Status": [COMPLAINT_STATUS[h['status']]['label'] for h in history],
            "Date": [h['timestamp'] for h in history]
        })
